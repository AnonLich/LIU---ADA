...
procedure Main is
   ...
   type AtoG is array (Character range 'a' .. 'g') of Integer;

   AtoG1 : constant AtoG := (1, 7, 12, 42, 112, 365, 1_024);
   ...
   begin
      ...
      Put_Line
        (F1,
         "|Inläst sträng       |Korrigerat vikingskt tal       |Värde     |");
      Lines (F1);
      New_Line (F1);
      Open (F, In_File, Argument (1));
   end Create_Open_Files;

   procedure Sum_Corrections
     (Corrected_Sum, Biggest_Integer : in out Integer; I : in Integer;
      Biggest_Viking, S              : in out String)
   is
   begin
   ...
   procedure Write_To_OutputFile
     (F1 : out File_Type; S : in out String; I, Corrected_Sum : in out Integer)
   is
   begin
      Put (F1, "|");
      Put (F1, S (1 .. I));
   ...
   procedure Print_To_Console
     (Biggest_Integer : in Integer; Biggest_Viking : in String)
   is
   begin
...
begin
   ...
   while not End_Of_File (F) loop
      ...
      if Check_If_Viking (S (1 .. I)) then
         Sum_Corrections
           (Corrected_Sum, Biggest_Integer, I, Biggest_Viking, S);
